...
7. **Render + VERIFY BY SCREENSHOT** (never skip): `cd remotion && node scripts/render-all.mjs --scale=1 <ShotIds>` (opaque → `out/<id>.mp4`, transparent overlays → `.mov`). Validate with a still first (`--still`); for word-synced reveals/scrolls, render the clip and pull a frame at **each cue** (`ffmpeg -ss <t> -i out/<id>.mp4 -frames:v 1 f.jpg`) — one 60% still is not enough. Then machine-triage with `python tools/verify_frames.py videos/<project> <ShotId> --cues` — declare `QA_CUES = [{ frame, expect }]` in the shot and it renders those frames, runs structural checks + golden-frame regression, and with `--ai` asserts each `expect` via Gemini. `--approve` sets the golden once the shot looks right. It triages where to look; it does not replace your eye.
...
- **Recording vs TSX.** TSX when you want controlled highlight/scroll/zoom; a real screen recording only when the point is genuine PROOF (real output, real speed). Leave a clearly-noted placeholder slot for recordings the user will provide. Real recordings now have a first-class path: the `/real-screencast` skill wraps an .mp4 in the branded chrome (zoom, highlights, click pulses) via `lib/realscreencast.tsx` — no placeholder needed once the capture exists.
