...
- **Real screen recording (`real-screencast`)** — when the point is genuine **proof**: real latency, a live result appearing, something you can't fake credibly. Real captures now have a first-class path — the `real-screencast` skill flows an .mp4 through the same branded, word-synced chrome (browser frame, zoom, highlight, click pulses) via `lib/realscreencast.tsx`. Faking is a choice, not the only mode.
...
## Step 1b — or CAPTURE them (auto-geometry, no by-hand fractions)

If the UI is drivable (your app, or VS Code via serve-web), skip the manual fraction
math entirely. `tools/capture_web.py` drives real Chromium through a choreography and,
for every `click`/`fill`, records the target element's exact center as a **viewport
fraction** into `manifest.json` (`interactions[]`). Then `tools/gen_screencast.py`
compiles that manifest straight into a shot — PAGES, the CURSOR path, and CLICKS all
pre-computed (cursor arrives 2f before each click; navigation vs in-page filter
inferred from the URLs), leaving you to *tune* instead of *measure*.

One-command flow (frame 0 = the shot's master_in_s):

    python tools/capture_web.py --spec videos/<p>/work/<svc>.capture.json
    python tools/gen_screencast.py \
      --manifest media/projects/<p>/<svc>/manifest.json \
      --name <ShotName> --out remotion/src/shots/<p>/<ShotName>.gen.tsx --fps 60 \
      --transcript videos/<p>/work/edited-transcript.json \
      --cue-words "workers,flux"     # pins each page-change to a word's start

`--cue-words` maps page-change N to the Nth word's start time (ms→frames). Omit the
transcript to get evenly-spaced cues with RETIME TODOs. The emitted `*.gen.tsx` is a
**starting point** ("tune freely" header): render, look, and nudge fractions/frames as
Step 4 already prescribes; add the ken-burns `zoom` on the payoff by hand (a TODO for it
is emitted on the last page). Then `cd remotion && npm run gen`.
...
- **Machine-triage first**: `python tools/verify_frames.py videos/<project> <CompId> --cues` renders every `QA_CUES` frame and runs structural + golden-frame checks (`--ai` to assert each cue's `expect` semantically — cursor-on-target, right URL, right payoff). Set the `expect` strings to the exact moments you'd verify by eye; `--approve` once it looks right, and re-renders gate against the golden.
