...
**Machine-triage the frame-read with `verify_frames.py`.** Declare the QA frames in the shot itself so the check is reproducible — export a `QA_CUES` array right after `compositionConfig`:
...
Then `python tools/verify_frames.py videos/<project> <CompId> --cues` renders those frames (via `scripts/frames.mjs`) and runs structural checks (blank / black / white / decode) + golden-frame regression, and with `--ai` asserts each `expect` semantically via Gemini. It triages *where* to look — it does not replace your eye. First time a shot looks right, `--approve` to set the golden baseline; later re-renders compare against it. `QA_CUES` is machine-readable intent, replacing ad-hoc `const CUE = 240; // "…"` comments.
